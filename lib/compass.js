  ...
  AK8963: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        state.calibration = {
          x: { offset: null, max: 0, min: 0 },
          y: { offset: null, max: 0, min: 0 },
          z: { offset: null, max: 0, min: 0 },
        };

        if (opts.offsets) {
          state.isPreCalibrated = true;
          state.calibration.x.offset = opts.offsets[0];
          state.calibration.y.offset = opts.offsets[1];
          state.calibration.z.offset = opts.offsets[2];
        }
        ...
        var read = function() {
          if (state.isCalibrated || state.isPreCalibrated) {
            interval = 20;
          }
              ...
              if (!state.isCalibrated) {
                if (state.isPreCalibrated) {
                  ...
                  if (samples) {
                    ...
                    state.calibration.x.offset = (calibration.x.max + calibration.x.min) / 2;
                    state.calibration.y.offset = (calibration.y.max + calibration.y.min) / 2;
                    state.calibration.z.offset = (calibration.z.max + calibration.z.min) / 2;
                ...
                if (state.isCalibrated) {
                  this.emit("calibrated", [
                    state.calibration.x.offset,
                    state.calibration.y.offset,
                    state.calibration.z.offset,
                  ]);
                }
              }
    ...
    toScaledHeading: {
      value: function(data) {
        var x = (data.x * data.scale) - data.calibration.x.offset;
        var y = (data.y * data.scale) - data.calibration.y.offset;
