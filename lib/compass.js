  ...
  AK8963: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        // Because this compass is most likely used as part of the
        // MPU9250 9DOF sensor package, we need to ensure that the
        // correct address for AK8963 is used. Since the IMU will
        // forward a options from the initialization of a MPU9250
        // controller, we need to swap the addressing around.
        var mpu9250Address = 0x68;

        // When the forwarded address matches a valid MPU9250
        // address, then it's safe to assume the component is
        // part of a package, in which case we need to save
        // the forwarded address to use when enabling the
        // "pass through" command.
        if (opts.address &&
            (opts.address === 0x68 || opts.address === 0x69)) {
          // Save a "copy" of the forwarded address
          mpu9250Address = opts.address;
          // Nulling the address means the default will
          // be used below.
          opts.address = null;
        }

        var address = opts.address || this.ADDRESSES[0];
        var autoCalibrate = typeof opts.autoCalibrate !== "undefined" ? opts.autoCalibrate : true;
        // AK8963 Datasheet
        // http://www.akm.com/akm/en/file/datasheet/AK8963.pdf
        //
        // AK8963 Firmware
        // http://lxr.free-electrons.com/source/drivers/iio/magnetometer/ak8975.c

        // TODO: Make this user definable in next iteration
        state.scale = 0.29;
        state.samples = 500;
        state.isCalibrated = false;
        state.autoCalibrate = autoCalibrate;
        state.calibration = {
          x: { value: null, max: 0, min: 0, previous: null },
          y: { value: null, max: 0, min: 0, previous: null },
          z: { value: null, max: 0, min: 0, previous: null },
        };

        if (opts.calibration) {
          state.autoCalibrate = false;
          state.calibration.x.value = opts.calibration[0];
          state.calibration.y.value = opts.calibration[1];
          state.calibration.z.value = opts.calibration[2];
        }

        // If the MPU9250 controller is active, we
        // need to ensure that it's active. Some
        // platforms require an i2cConfig call for
        // each active I2C peripheral, by address.
        this.io.i2cConfig({
          address: mpu9250Address
        });

        // Proceed with normal I2C peripheral component initialization
        opts.address = address;
        ...
        // BEN => Bypass Enable
        //
        // var fsync = 0b00000010;
        var fsync = 0x02;
        this.io.i2cWrite(mpu9250Address, this.REGISTER.INTERRUPT, fsync);

        // See 8.3.6. CNTL1: Control1
        //
        //
        // BITS:
        //    7   6   5   4   3   2   1   0
        ...
        //  765  4  3210
        // [000][B][0000]: Power-down mode
        // [000][B][0001]: Single measurement mode
        // [000][B][0010]: Continuous measurement mode 1
        // [000][B][0110]: Continuous measurement mode 2
        // [000][B][0100]: External trigger measurement mode
        // [000][B][1000]: Self-test mode
        // [000][B][1111]: Fuse ROM access mode

        // BIT[4]: Output bit setting
        //  765  4  3210
        // [000][0][MMMM]: 14-bit output
        // [000][1][MMMM]: 16-bit output
        //
        // CONTROL: 00010110
        //
        // var control = 0b00010110;
        // var control = 0x16;
        //
        // var control = 0b00010001;
        var interval = 0;
        var control = 0x01;

        var read = function() {
          if (state.isCalibrated || state.autoCalibrate) {
            interval = 20;
          }
          // Enable Magnetometer in Single measurement mode, 14-bit
          this.io.i2cWrite(address, this.REGISTER.CONTROL1, control);

          setTimeout(function() {
            this.io.i2cReadOnce(address, this.REGISTER.READ, READLENGTH, function(bytes) {
              var data = {
              ...
              if (!state.isCalibrated) {
                if (!state.autoCalibrate) {
                  ...
                  if (state.samples) {
                    state.samples--;
                    state.calibration.x.max = Math.max(state.calibration.x.max, data.x);
                    state.calibration.y.max = Math.max(state.calibration.y.max, data.y);
                    state.calibration.z.max = Math.max(state.calibration.z.max, data.z);

                    state.calibration.x.min = Math.min(state.calibration.x.min, data.x);
                    state.calibration.y.min = Math.min(state.calibration.y.min, data.y);
                    state.calibration.z.min = Math.min(state.calibration.z.min, data.z);
                  } else {
                    var calibration = state.calibration;

                    state.calibration.x.value = (calibration.x.max + calibration.x.min) / 2;
                    state.calibration.y.value = (calibration.y.max + calibration.y.min) / 2;
                    state.calibration.z.value = (calibration.z.max + calibration.z.min) / 2;

                    // If previously calibrated,
                    if (state.calibration.x.previous !== null &&
                        state.calibration.y.previous !== null &&
                        state.calibration.z.previous !== null) {

                      state.calibration.x.value = (state.calibration.x.previous +
                                                    state.calibration.x.value) / 2;
                      state.calibration.y.value = (state.calibration.y.previous +
                                                    state.calibration.y.value) / 2;
                      state.calibration.z.value = (state.calibration.z.previous +
                                                    state.calibration.z.value) / 2;
                    }
                    state.isCalibrated = true;
                  }
                }
                ...
                if (state.isCalibrated) {
                  this.emit("calibrated", [
                    state.calibration.x.value,
                    state.calibration.y.value,
                    state.calibration.z.value,
                  ]);
                }
              }
    ...
    calibrate: {
      value: function() {
        ...
        state.calibration.x.previous = state.calibration.x.value;
        state.calibration.y.previous = state.calibration.y.value;
        state.calibration.z.previous = state.calibration.z.value;

        // Reset the calibration state, this will allow the data handler
        // to re-enter calibration mode above.
        state.samples = 100;
        state.isCalibrated = false;
        state.autoCalibrate = true;
      ...
      value: function(predicate) {
        ...
        var loop = function() {
          ...
          this.once("calibrated", function() {
            ...
            if (!predicate(null, count * samples, elapsed)) {
              loop();
            }
          });
        }.bind(this);
    ...
    toScaledHeading: {
      value: function(data) {
        var x = (data.x * data.scale) - data.calibration.x.value;
        var y = (data.y * data.scale) - data.calibration.y.value;
